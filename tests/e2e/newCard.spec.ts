...
test('添加新卡片并移到进行中', async ({ page }) => {
  // 1. 用户打开看板应用
  await page.goto('http://localhost:8001/yeahMyKanBan/kanBan')

  // 2. 用户添加新卡片
  // 2.1 点击“添加新卡片”
  const todoListColumn = page.locator('data-testid=kanban-column', {
    hasText: '待处理',
  })
  ...
  const ongoingListColumn = page.locator('data-testid=kanban-column', {
    hasText: '进行中',
  })
  await newCard.dragTo(ongoingListColumn)
  await expect(todoListColumn.locator('li').first()).not.toContainText(
    '测试E2E任务-1',
  )
  await expect(ongoingListColumn.locator('li').first()).toContainText(
    '测试E2E任务-1',
  )
